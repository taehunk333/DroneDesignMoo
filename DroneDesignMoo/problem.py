...
class DroneOptimizationProblem(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        mtotal = MASS_PAYLOAD + mframe + mbattery + nmotors * mmotor
        Aprop = (np.pi * Dprop ** 2) / 4

        Tmotor_max = KT * Dprop ** 3 * Pprop * KV_motor * Vbattery
        Phover = (mtotal * GRAV_CONSTANT) ** 1.5 / np.sqrt(2 * AIR_DENSITY * nmotors * Aprop)
        tflight = (Cbattery * Vbattery * ETA_MOTOR * ETA_ESC) / (1000 * Phover)

        T_W = (nmotors * Tmotor_max) / (mtotal * GRAV_CONSTANT)
